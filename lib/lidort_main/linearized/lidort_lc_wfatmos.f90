...
!  2/28/21. Version 3.8.3. LCS Convergence routines moved to new module LCS_CONVERGE
!  2/28/21. Version 3.8.3. I/O lists substantially reorganized. ( UPUSER_COLUMNWF, DNUSER_COLUMNWF )
...
SUBROUTINE UPUSER_COLUMNWF ( &
             ...
             NSTREAMS, NLAYERS, N_PPSTREAMS, PPSTREAM_MASK, N_USER_LEVELS, NV_PARAMETERS, & ! Input numbers (basic)
             ...
             L_XNEG, L_U_XPOS, L_U_XNEG, LC_U_WPOS, L_ATERM_SAVE, L_BTERM_SAVE,           & ! Input Linearized PIs
...
!    -- output arrays are now LC_UT_GFUNC_UP, LC_UT_GFUNC_DN.
!    -- Code changes to use ordinary (non-logarithmic) derivatives L_ATERM_SAVE, L_BTERM_SAVE
...
!    -- 2/28/21. Version 3.8.3. New output
      ...
      REAL(fpk), intent(inout) :: LC_LAYER_MSSTS_F ( MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS )
...
!  Layer source term
            ...
            ( DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY, & ! input
              ...
              IB, N, NV_PARAMETERS, NSTREAMS, N_PPSTREAMS, PPSTREAM_MASK,    & ! input, Numbers
...
!    -- NOTE, MSSTS only available for OBSERVATION GEOMETRY !!!!!!!!!!!!!!!!!!!!!!!!1
            ...
            IF ( DO_MSSTS .and. DO_OBSERVATION_GEOMETRY) THEN
               DO Q = 1, NV_PARAMETERS
                  LC_LAYER_MSSTS_F(IB,N,Q) = FLUX_MULTIPLIER * L_LAYER_SOURCE(IB,Q)
               ENDDO
            ENDIF
...
!    -- Code changes to use ordinary (non-logarithmic) derivatives L_ATERM_SAVE, L_BTERM_SAVE
          ...
          IF ( DO_INCLUDE_MVOUTPUT ) THEN
           ...
           ( DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY,                    & ! Input
             TAYLOR_ORDER, NSTREAMS, NLAYERS, IB, UTA, UT, N, NV_PARAMETERS,                   & ! Input
...
!  Part-layer source term
           ...
           ( DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY,       & ! input, Flags
             DO_MSMODE_LIDORT, SOURCETERM_FLAG, M, TAYLOR_ORDER,                  & ! input, FLags + Order
             IB, UT, N, NV_PARAMETERS, NSTREAMS, N_PPSTREAMS, PPSTREAM_MASK,      & ! input, Numbers
...
!    DO_INCLUDE_MVOUTPUT = .TRUE. only for Fourier = 0 )
          ...
          IF ( DO_INCLUDE_MVOUTPUT ) THEN
            ...
            ( DO_THERMAL_TRANSONLY,  FLUX_MULTIPLIER,                & ! input
              NLAYERS, NSTREAMS, IB, UTA, NLEVEL, NV_PARAMETERS,      & ! input
...
SUBROUTINE DNUSER_COLUMNWF ( &
             ...
             DO_SOLAR_SOURCES,  DO_INCLUDE_THERMEMISS,   DO_THERMAL_TRANSONLY,            & ! Input flags (sources)
             DO_MSMODE_LIDORT,  DO_LAYER_SCATTERING,     DO_MSSTS, FOURIER, IBEAM,        & ! Input flags/numbers (basic)
             NSTREAMS, N_PPSTREAMS, PPSTREAM_MASK, N_USER_LEVELS, NV_PARAMETERS,          & ! Input numbers (basic)
             ...
             L_KEIGEN, L_T_DELT_EIGEN, L_T_UTUP_EIGEN, L_T_UTDN_EIGEN, L_XPOS,            & ! Input Linearized Homog solutions
             L_XNEG,L_U_XPOS, L_U_XNEG, LC_U_WNEG, L_ATERM_SAVE, L_BTERM_SAVE,            & ! Input Linearized PIs
...
!    -- Code changes to use ordinary (non-logarithmic) derivatives L_ATERM_SAVE, L_BTERM_SAVE
          ...
          IF ( DO_INCLUDE_MVOUTPUT ) THEN
            ...
            ( DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY,                 & ! Input
              TAYLOR_ORDER, NSTREAMS, IB, UTA, UT, N, NV_PARAMETERS,                         & ! Input
              QUAD_STREAMS, FLUX_MULTIPLIER, PARTAU_VERT, L_DELTAU_VERT, BEAM_CUTOFF,        & ! Input
...
!    DO_INCLUDE_MVOUTPUT = .TRUE. only for Fourier = 0 )
          ...
          IF ( DO_INCLUDE_MVOUTPUT ) THEN
            ...
            ( DO_THERMAL_TRANSONLY, FLUX_MULTIPLIER,      & ! Input
              NSTREAMS, IB, UTA, NLEVEL, NV_PARAMETERS,   & ! Input
...
SUBROUTINE MIFLUX_COLUMNWF                                            &
           ( DO_UPWELLING, DO_DNWELLING, DO_INCLUDE_DIRECTBEAM,       & ! Input
             IB, NV_PARAMETERS, NSTREAMS, N_USER_LEVELS, FLUX_FACTOR, & ! Input
...
SUBROUTINE GET_LC_TOASOURCE &
     ( N_PPSTREAMS, PPSTREAM_MASK, IBEAM, NV_PARAMETERS, LC_TOA_SOURCE )
...
SUBROUTINE GET_LC_BOASOURCE &
     ( DO_USER_STREAMS, DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS,       & ! Input flags sources
       ...
       NSTREAMS, NLAYERS, IBEAM, FOURIER, NV_PARAMETERS,               & ! Input numbers
...
!  ---------------
      ...
      LOGICAL   :: DO_QTHTONLY
      INTEGER   :: M, N, I, UM, AA, Q, IB, LAY, LUM
...
!  initialise linearized BOA source functions
      ...
      IF ( DO_USER_STREAMS ) THEN
         DO LUM = 1, N_PPSTREAMS
            UM = PPSTREAM_MASK(LUM,IBEAM)
            L_BOA_MSSOURCE(UM,1:NV_PARAMETERS) = ZERO
            L_BOA_DBSOURCE(UM,1:NV_PARAMETERS) = ZERO
         ENDDO
      ENDIF
...
!  2/28/21. Version 3.8.3. BRDF Fourier arrays defined locally, no Fourier index
