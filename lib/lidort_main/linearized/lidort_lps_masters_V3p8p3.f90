...
!    -- Converge routines have been moved to their own modules (lidort_converge.f90, lidort_lps_converge.f90)
...
SUBROUTINE LIDORT_LPS_master ( do_debug_input, &
        LIDORT_FixIn,    & ! INPUTS
...
!  2/28/21. Version 3.8.3. Use LIDORT_LPS_CONVERGE_m for the three convergence routines

   USE lidort_lps_converge_m  , only : LIDORT_LPS_CONVERGE, LIDORT_LPS_CONVERGE_OBSGEO, LIDORT_LPS_CONVERGE_DOUBLET
...
!  Total number of profile and surface Jacobians

      INTEGER :: N_TOTALPROFILE_WFS, N_TOTALCOLUMN_WFS
...
!    ==> MSST situation expanded to include Downwelling case (as an alternative)
      ...
      real(fpk) :: LP_SURF_MSSTS_F  ( MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS )
      real(fpk) :: LP_LAYER_MSSTS_F ( MAXBEAMS, MAXLAYERS, MAXLAYERS, MAX_ATMOSWFS )
...
!  Fourier-component Profile weighting functions at user angles
      ...
      REAL(fpk)       :: PROFILEWF_F ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_USER_STREAMS,MAXBEAMS, MAX_DIRECTIONS)
...
!    ==> Added LOSTRANS_DN for the Downwelling Alternative.
      ...
      real(fpk) :: FO_LP_LOSTRANS_UP ( MAX_GEOMETRIES, MAXLAYERS, MAX_ATMOSWFS )
      real(fpk) :: FO_LP_LOSTRANS_DN ( MAX_GEOMETRIES, MAXLAYERS, MAX_ATMOSWFS )
...
!  Local integers
      ...
      INTEGER         :: OFF, K, N, Q, Q1, UA, UM, IB, UT, G, G1, G2, UTA, LUM, LUA, V, NMOMS, NMOMSINP, NWFS
...
!  2/28/21. Verison 3.8.3. define local linearization flag
      ...
      DO_LINEARIZATION             = DO_SURFACE_LINEARIZATION .or. DO_PROFILE_LINEARIZATION
...
!   -- 2/28/21. Version 3.8.3. Local arrays no longer required. Type structure is filled directly.
...
!    --Type structure arrays filled directly after SFO call, but zeroed here
      ...
      IF ( .not.DO_FOCORR_EXTERNAL ) THEN
         LIDORT_LinSup%SS%Atmos%TS_PROFILEWF_SS(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,:,:) = ZERO
         LIDORT_LinSup%SS%Atmos%TS_PROFILEWF_DB(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,:)   = ZERO
...
!  2/28/21. Version 3.8.3. Add DO_DOUBLET Geometry flag, plus offset input arguments
            ...
            CALL SFO_LPS_MASTER_INTERFACE ( &
               ...
               DO_PLANE_PARALLEL, DO_FOCORR_NADIR, DO_FOCORR_OUTGOING, DO_SSCORR_USEPHASFUNC,       & ! Input SS control flags
               DO_DELTAM_SCALING, DO_UPWELLING, DO_DNWELLING, DO_PARTLAYERS,DO_OBSERVATION_GEOMETRY,& ! input Model control flags
               DO_DOUBLET_GEOMETRY, DO_BRDF_SURFACE, DO_SURFACE_LEAVING, DO_WATER_LEAVING,          & ! Input Optical/Surface flags
               DO_SL_ISOTROPIC,                                                                     & ! Input Optical/Surface flags
...
!    -- fully vectorized LIDORT_Sup INTENSITY_SS/DB with eliminated geometry loop
            ...
            if ( do_upwelling ) then
               ...
               enddo
               IF ( DO_PROFILE_LINEARIZATION ) THEN
                  LIDORT_LinSup%SS%Atmos%TS_PROFILEWF_SS(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,UPIDX) = &
                             FO_PROFILEWF_ATMOS(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,UPIDX)
                  LIDORT_LinSup%SS%Atmos%TS_PROFILEWF_DB(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES)       = &
            ...
            if ( do_dnwelling ) then
               ...
               IF ( DO_PROFILE_LINEARIZATION ) THEN
                  LIDORT_LinSup%SS%Atmos%TS_PROFILEWF_SS(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,DNIDX) = &
                             FO_PROFILEWF_ATMOS(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,DNIDX)
               ENDIF
            endif
...
!    ==> [output for Multiple scatter Sphericity corrections, filled directly in Converge routines]
      ...
      IF ( DO_MSSTS ) THEN
         ...
         IF ( DO_UPWELLING ) THEN
            ...
            IF ( do_PROFILE_LINEARIZATION ) THEN
               DO Q = 1, N_TOTALPROFILE_WFS
                  LIDORT_LinOut%Atmos%TS_LP_LOSTRANS(Q,1:NBEAMS,1:NLAYERS) = FO_LP_LOSTRANS_UP(1:NBEAMS,1:NLAYERS,Q)
               ENDDO
            ENDIF
         ELSE IF ( DO_DNWELLING ) THEN
            ...
            IF ( do_PROFILE_LINEARIZATION ) THEN
               DO Q = 1, N_TOTALPROFILE_WFS
                  LIDORT_LinOut%Atmos%TS_LP_LOSTRANS(Q,1:NBEAMS,1:NLAYERS) = FO_LP_LOSTRANS_DN(1:NBEAMS,1:NLAYERS,Q)
               ENDDO
            ENDIF
         ENDIF
      ENDIF
...
!    -- Include flag DO_MSSTS (line 7) and MSSTS outputs LAYER_MSSTS_F, SURF_MSSTS_F, and their 4 linearizations
!    -- Include flag DO_TOACONTRIBS and input CUMTRANS, output is M_CONTRIBS_F
!    -- Include ASYMTX Tolerance variable to the list (Line 7)
!    -- removed HELP_AQ, HELP_BQ arrays (not needed)
           CALL LIDORT_LPS_FOURIER ( FOURIER, &
            ...
            DO_PLANE_PARALLEL, DO_MSMODE_LIDORT, DO_MULTIBEAM, LOCAL_DO_ALBTRN_MEDIA,               & !Input flags (Beam/Planetary)
            LOCAL_DO_PLANETARY_PROBLEM,                                                             & !Input flags (Beam/Planetary)
            ...
            DO_PROFILE_LINEARIZATION, DO_SURFACE_LINEARIZATION, DO_SLEAVE_WFS, DO_ATMOS_LBBF, DO_SURFACE_LBBF, & !Input flags (Lin)
            NSTREAMS, NLAYERS, NBEAMS, N_USER_STREAMS, N_USER_LEVELS, N_THERMAL_COEFFS, NMOMENTS,   & !Input (Control Numbers)
            LAYER_VARY_FLAG, LAYER_VARY_NUMBER, N_SURFACE_WFS, N_SLEAVE_WFS,                        & !Input (Lin Numbers)
            NSTREAMS_2, NTOTAL, N_SUBDIAG, N_SUPDIAG, N_PARTLAYERS, N_PPSTREAMS, PPSTREAM_MASK,     & !Input (Bookkeep Numbers)
            ...
            DELTAU_VERT, PARTAU_VERT, DELTAU_POWER, XTAU_POWER, OMEGA_MOMS, DELTAU_SLANT,           & !Input (Atmosphere optical)
            ...
            LP_EMULT_UP, LP_EMULT_DN, LP_UT_EMULT_UP, LP_UT_EMULT_DN,                               & !Input LP Trans.
            INTENSITY_F, MEANI_DIFFUSE, FLUX_DIFFUSE, DNMEANI_DIRECT, DNFLUX_DIRECT,                & !Output MAIN I/FLUXES
            PROFILEWF_F, MEANI_DIFFUSE_PROFWF, FLUX_DIFFUSE_PROFWF, DNMEANI_DIRECT_PROFWF,          & !Output MAIN LP Jacs
            DNFLUX_DIRECT_PROFWF,                                                                   & !Output MAIN LP Jacs
            ...
            LP_TRANS_ATMOS_FINAL, LP_TRANSBEAM, LP_ALBMED_USER, LP_ALBMED_FLUXES,                   & !Output LP TRANS MEDIA
            LP_TRNMED_USER, LP_TRNMED_FLUXES,                                                       & !Output LP TRANS MEDIA
            MS_CONTRIBS_F, LAYER_MSSTS_F, SURF_MSSTS_F,                                             & !Output SPECIALIST
            LP_LAYER_MSSTS_F, LP_SURF_MSSTS_F, LS_LAYER_MSSTS_F, LS_SURF_MSSTS_F,                   & !Output SPECIALIST
            STATUS_SUB, MESSAGE, TRACE_1, TRACE_2, TRACE_3 )                                          !Output Status

!  error handling

          IF ( STATUS_SUB .NE. LIDORT_SUCCESS ) THEN
            if ( Len(TRACE_3) == 0 ) TRACE_3 = ' ** Called in LIDORT_LPS_MASTER'
            STATUS_CALCULATION = LIDORT_SERIOUS
            LIDORT_Out%Status%TS_STATUS_CALCULATION = STATUS_CALCULATION
            LIDORT_Out%Status%TS_MESSAGE = MESSAGE
            LIDORT_Out%Status%TS_TRACE_1 = TRACE_1
...
! debug
!           write(45,*) FOURIER, INTENSITY_F (1:5, 1, 1, 1 )
...
!  2/28/21. Version 3.8.3. Add Doublet geometry optionAdd FOCORR_EXTERNAL to if clause
                    ...
                    if ( layer_vary_flag(k) ) then
                      ...
                      do uta = 1, n_user_levels
                        DO Q = 1, NWFS
                          ...
                          LIDORT_LinSup%SS%Atmos%TS_PROFILEWF_DB(Q,K,UTA,IB) = &
                                LIDORT_LinSup%SS%Atmos%TS_PROFILEWF_DB(Q,K,UTA,IB) + L_CUMSOURCE_DB
                        enddo
                      enddo
                    endif
                  enddo
...
!    -- Use/Fill linearized type structure variables directly (replaces PROFILEWF, SURFACEWF, associated FO inputs)
...
!  Fourier summation of linearization quantities
                ...
                IF ( DO_LINEARIZATION ) THEN
                  CALL LIDORT_LPS_CONVERGE_OBSGEO ( &
                    ...
                    LP_LAYER_MSSTS_F, LP_SURF_MSSTS_F, LS_LAYER_MSSTS_F, LS_SURF_MSSTS_F, & ! Input Jacobian MSST inputs
...
!  Fourier summation of linearization quantities
                ...
                IF ( DO_LINEARIZATION ) THEN
                  CALL LIDORT_LPS_CONVERGE_DOUBLET ( &
                    DO_FOCORR, DO_FOCORR_ALONE, DO_UPWELLING, DO_NO_AZIMUTH,              & ! Input flags
                    DO_PROFILE_LINEARIZATION, DO_SURFACE_LINEARIZATION, DO_BRDF_SURFACE,  & ! Input flags
                    ...
                    PROFILEWF_F, SURFACEWF_F, LIDORT_LinSup%SS, LIDORT_LinOut )             ! Input Azm, fields
                ENDIF
...
!  Fourier summation of linearization quantities
                ...
                IF ( DO_LINEARIZATION ) THEN
                  CALL LIDORT_LPS_CONVERGE ( &
                    DO_FOCORR, DO_FOCORR_ALONE, DO_UPWELLING, DO_NO_AZIMUTH,              & ! Input flags
                    DO_PROFILE_LINEARIZATION, DO_SURFACE_LINEARIZATION, DO_BRDF_SURFACE,  & ! Input flags
                    IBEAM, FOURIER, NLAYERS, N_USER_STREAMS, N_USER_LEVELS, N_DIRECTIONS, & ! Input control numbers
                    LAYER_VARY_FLAG, LAYER_VARY_NUMBER, N_SURFACE_WFS, N_SLEAVE_WFS,      & ! Input linearization control
                    VZA_OFFSETS, WHICH_DIRECTIONS, LOCAL_N_USERAZM, AZMFAC,               & ! Input bookkeeping
                    PROFILEWF_F, SURFACEWF_F, LIDORT_LinSup%SS, LIDORT_LinOut )             ! Input/Output fields
                ENDIF
...
!  2/28/21. Version 3.8.3. Main Profile WFs now filled directly
            ...
            DO IB =1, NBEAMS ; DO UM = 1, N_USER_STREAMS
               ...
               DO N = 1, NLAYERS
                  DO Q = 1, LAYER_VARY_NUMBER(N)
                     ...
                     LIDORT_LinOut%Atmos%TS_PLANETARY_TRANSTERM_PROFWF(OFF+UM,N,Q) = L_TRANS(Q)
                  ENDDO
               ENDDO
            ENDDO ; ENDDO
         ELSE
...
SUBROUTINE LIDORT_LPS_FOURIER ( FOURIER, &
            ...
            DO_SOLAR_SOURCES, DO_REFRACTIVE_GEOMETRY, DO_ADDITIONAL_MVOUT, DO_MVOUT_ONLY,           & !Input flags (RT operation)
            DO_LAYER_SCATTERING, DO_SOLUTION_SAVING, DO_BVP_TELESCOPING, DO_BVTEL_INITIAL,          & !Input flags (Performance)
            DO_PLANE_PARALLEL, DO_MSMODE_LIDORT, DO_MULTIBEAM, DO_ALBTRN_MEDIA, DO_PLANETARY_PROBLEM,& !Input flags (Beam/Planetary)
            DO_MSMODE_THERMAL, DO_THERMAL_EMISSION, DO_SURFACE_EMISSION, DO_THERMAL_TRANSONLY,      & !Input flags (Thermal)
            DO_MSSTS, DO_TOA_CONTRIBS, DO_TOAFLUX, DO_BOAFLUX,                                & !Input flags (MSST/Specialist)
            DO_BRDF_SURFACE, DO_SURFACE_LEAVING, DO_SL_ISOTROPIC, DO_REFLECTED_DIRECTBEAM,          & !Input flags (Surface)
            DO_WATER_LEAVING, DO_EXTERNAL_WLEAVE, DO_TF_ITERATION,                                  & !Input flags (Water-leaving)
            DO_PROFILE_LINEARIZATION, DO_SURFACE_LINEARIZATION, DO_SLEAVE_WFS, DO_ATMOS_LBBF, DO_SURFACE_LBBF, & !Input flags (Lin)
            NSTREAMS, NLAYERS, NBEAMS, N_USER_STREAMS, N_USER_LEVELS, N_THERMAL_COEFFS, NMOMENTS,   & !Input (Control Numbers)
            LAYER_VARY_FLAG, LAYER_VARY_NUMBER, N_SURFACE_WFS, N_SLEAVE_WFS,                        & !Input (Linearization Numbers)
            ...
            L_T_DELT_DISORDS, L_T_DISORDS_UTUP, L_T_DISORDS_UTDN,                                        & ! Input L Trans.
            LP_EMULT_UP, LP_EMULT_DN, LP_UT_EMULT_UP, LP_UT_EMULT_DN,                                    & ! Input LP Trans.
            INTENSITY_F, MEANI_DIFFUSE, FLUX_DIFFUSE, DNMEANI_DIRECT, DNFLUX_DIRECT,                            & !Out MAIN I/FLUXES
            PROFILEWF_F, MEANI_DIFFUSE_PROFWF, FLUX_DIFFUSE_PROFWF, DNMEANI_DIRECT_PROFWF, DNFLUX_DIRECT_PROFWF,& !Out MAIN LP Jacs
            SURFACEWF_F, MEANI_DIFFUSE_SURFWF, FLUX_DIFFUSE_SURFWF,                                             & !Out MAIN LS Jacs
...
!    ==> MSST situation expanded to include Downwelling case (as an alternative)
      ...
      real(fpk), INTENT (OUT) :: LP_SURF_MSSTS_F  ( MAXBEAMS, MAXLAYERS, MAX_ATMOSWFS )
      real(fpk), INTENT (OUT) :: LP_LAYER_MSSTS_F ( MAXBEAMS, MAXLAYERS, MAXLAYERS, MAX_ATMOSWFS )
...
!    -- 2/28/21. Version 3.8.3. Rename UT_GMULT to UT_GFUNC
      ...
      REAL(fpk)  :: LP_UT_GFUNC_UP (MAXSTREAMS,MAX_PARTLAYERS,MAXLAYERS,MAX_ATMOSWFS)
      REAL(fpk)  :: LP_UT_GFUNC_DN (MAXSTREAMS,MAX_PARTLAYERS,MAXLAYERS,MAX_ATMOSWFS)
...
!  .. error tracing
            ...
            IF ( STATUS_SUB .NE. LIDORT_SUCCESS ) THEN
               write(CF,'(I2)')FOURIER
               TRACE_2 =  'LIDORT_QHOM_SOLUTION Called in LIDORT_LPS_FOURIER, Fourier component '//CF
...
!  .. error tracing
               ...
               IF ( STATUS_SUB .NE. LIDORT_SUCCESS ) THEN
                  write(CF,'(I2)')FOURIER
                  TRACE_2 =  'LIDORT_L_QHOM_SOLUTION Called in LIDORT_LPS_FOURIER, Fourier component '//CF
                  TRACE_3 =  'LIDORT_LPS_FOURIER call '
                  STATUS  = LIDORT_SERIOUS ; RETURN
               ENDIF
...
!    - 2/28/21. Version 3.8.3. BRDF_F argument defined locally for each Fourier.
         ...
         IF ( BVP_REGULAR_FLAG(FOURIER) ) THEN
            ...
            IF ( STATUS_SUB .NE. LIDORT_SUCCESS ) THEN
               write(CF,'(I2)')FOURIER
               TRACE_3 = 'Error from BVP_MATRIXSETUP_MASTER, '//'Called in LIDORT_LPS_FOURIER, Fourier # '//CF
               STATUS = LIDORT_SERIOUS ; RETURN
            ENDIF
...
!  Telescoped case. Generalized to handle BRDF surfaces. 5/24/16. Rob
         ...
         ELSE
            ...
            IF ( STATUS_SUB .NE. LIDORT_SUCCESS ) THEN
               write(CF,'(I2)')FOURIER
               TRACE_3 = 'Error from BVPTEL_MATRIXSETUP_MASTER, '//'Called in LIDORT_LPS_FOURIER, Fourier # '//CF
               STATUS = LIDORT_SERIOUS ; RETURN
            ENDIF
...
!    --  Important Note, Only for observational geometry
              ...
              IF ( DO_MSSTS .and. DO_OBSERVATION_GEOMETRY) THEN
                DO Q = 1, N_LAYER_WFS
                  LP_SURF_MSSTS_F(IBEAM,LAYER_TO_VARY,Q) = FLUX_MULTIPLIER * LP_BOA_MSSOURCE(IBEAM,Q)
                ENDDO
              ENDIF
...
!  2/28/21. Version 3.8.3.
!    -- MSST flag (DO_MSSTS) is now included, generates output LP_LAYER_MSSTS_F
...
!    -- Code changes to use ordinary (non-logarithmic) derivatives L_ATERM_SAVE, L_BTERM_SAVE
              ...
              CALL UPUSER_PROFILEWF ( &
                 ...
                 DO_SOLAR_SOURCES,  DO_THERMAL_EMISSION,     DO_THERMAL_TRANSONLY,               & ! Input flags (sources)
                 DO_MSMODE_LIDORT,  DO_LAYER_SCATTERING,     DO_MSSTS, FOURIER, IBEAM, NSTREAMS, & ! Input flags/numbers (basic)
                 NLAYERS, N_PPSTREAMS, PPSTREAM_MASK, N_USER_LEVELS, LAYER_TO_VARY, N_LAYER_WFS, & ! Input numbers (basic)
                 ...
                 L_XNEG,L_U_XPOS, L_U_XNEG, LP_U_WPOS1, L_ATERM_SAVE, L_BTERM_SAVE,              & ! Input Linearized PIs
...
!    -- Code changes to use ordinary (non-logarithmic) derivatives L_ATERM_SAVE, L_BTERM_SAVE
             ...
             CALL DNUSER_PROFILEWF ( &
                ...
                NSTREAMS, N_PPSTREAMS, PPSTREAM_MASK, N_USER_LEVELS, LAYER_TO_VARY, N_LAYER_WFS, & ! Input numbers (basic)
                ...
                L_KEIGEN, L_T_DELT_EIGEN, L_T_UTUP_EIGEN, L_T_UTDN_EIGEN, L_XPOS,                & ! Input Linearized Homog solutions
                L_XNEG,L_U_XPOS, L_U_XNEG, LP_U_WNEG1, L_ATERM_SAVE, L_BTERM_SAVE,               & ! Input Linearized PIs
